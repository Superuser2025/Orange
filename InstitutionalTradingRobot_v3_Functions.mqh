...
//+------------------------------------------------------------------+
//| STATE TRACKING FOR ACCURATE EVENT TIMESTAMPS                     |
//| Track when conditions START to avoid duplicate entries           |
//+------------------------------------------------------------------+
static bool g_ConsolidationActive = false;
static datetime g_ConsolidationStartTime = 0;
static bool g_MomentumSurgeActive = false;
static datetime g_MomentumSurgeStartTime = 0;
static bool g_MomentumSlowActive = false;
static datetime g_MomentumSlowStartTime = 0;
static bool g_LowVolumeActive = false;
static datetime g_LowVolumeStartTime = 0;
static bool g_HighVolumeActive = false;
static datetime g_HighVolumeStartTime = 0;
static string g_LastMarketStructure = "";
static datetime g_MarketStructureChangeTime = 0;
...
void DetermineBias()
{
    ...
    double close_price = iClose(_Symbol, g_PreferredTimeframe, 1);
...
void MapLiquidityZones()
{
    ...
    {
        double high = iHigh(_Symbol, g_PreferredTimeframe, i);
        double low = iLow(_Symbol, g_PreferredTimeframe, i);
        datetime time = iTime(_Symbol, g_PreferredTimeframe, i);

        // Check for swing high
        bool is_swing_high = true;
        for(int j = 1; j <= lookback; j++)
        {
            if(i-j < 0 || i+j >= Bars(_Symbol, g_PreferredTimeframe))
            {
                is_swing_high = false;
                break;
            }
            if(iHigh(_Symbol, g_PreferredTimeframe, i-j) >= high ||
               iHigh(_Symbol, g_PreferredTimeframe, i+j) >= high)
            {
                is_swing_high = false;
                break;
            }
        ...
        for(int j = 1; j <= lookback; j++)
        {
            ...
            if(iLow(_Symbol, g_PreferredTimeframe, i-j) <= low ||
               iLow(_Symbol, g_PreferredTimeframe, i+j) <= low)
            {
                is_swing_low = false;
                break;
            }
...
void DetectFairValueGaps()
{
    ...
    {
        double high1 = iHigh(_Symbol, g_PreferredTimeframe, i+1);
        double low1 = iLow(_Symbol, g_PreferredTimeframe, i+1);
        double high3 = iHigh(_Symbol, g_PreferredTimeframe, i-1);
        double low3 = iLow(_Symbol, g_PreferredTimeframe, i-1);

        // Bullish FVG
        if(low3 > high1 && (low3 - high1) >= min_gap)
        {
            fvg_zones[fvg_count].id = next_fvg_id++;
            fvg_zones[fvg_count].top = low3;
            fvg_zones[fvg_count].bottom = high1;
            fvg_zones[fvg_count].time = iTime(_Symbol, g_PreferredTimeframe, i);
...
void DetectOrderBlocks()
{
    ...
    {
        double close_prev = iClose(_Symbol, g_PreferredTimeframe, i);
        double open_prev = iOpen(_Symbol, g_PreferredTimeframe, i);
        double close_curr = iClose(_Symbol, g_PreferredTimeframe, i-1);

        // Bullish Order Block
        if(close_prev < open_prev && close_curr > close_prev)
        {
            order_blocks[ob_count].id = next_ob_id++;
            order_blocks[ob_count].top = MathMax(open_prev, close_prev);
            order_blocks[ob_count].bottom = MathMin(open_prev, close_prev);
            order_blocks[ob_count].time = iTime(_Symbol, g_PreferredTimeframe, i);
...
{
    double current_price = iClose(_Symbol, g_PreferredTimeframe, 0);
...
void AnalyzeVolume()
{
    long current_vol = iVolume(_Symbol, g_PreferredTimeframe, 1);
...
{
    double current_high = iHigh(_Symbol, g_PreferredTimeframe, 1);
    double current_low = iLow(_Symbol, g_PreferredTimeframe, 1);
    double current_close = iClose(_Symbol, g_PreferredTimeframe, 1);
...
{
    double high1 = iHigh(_Symbol, g_PreferredTimeframe, 1);
    double close1 = iClose(_Symbol, g_PreferredTimeframe, 1);
...
double GetSwingHigh(int lookback)
{
    ...
    {
        double h = iHigh(_Symbol, g_PreferredTimeframe, i);
        if(h > high) high = h;
    }
    return high;
}
...
double GetSwingLow(int lookback)
{
    ...
    for(int i = 1; i <= lookback; i++)
    {
        double l = iLow(_Symbol, g_PreferredTimeframe, i);
        if(l < low) low = l;
    }
    return low;
}
...
{
    double open0 = iOpen(_Symbol, g_PreferredTimeframe, 1);
    double high0 = iHigh(_Symbol, g_PreferredTimeframe, 1);
    double low0 = iLow(_Symbol, g_PreferredTimeframe, 1);
    double close0 = iClose(_Symbol, g_PreferredTimeframe, 1);
    double body0 = MathAbs(close0 - open0);
    double range0 = high0 - low0;

    double open1 = iOpen(_Symbol, g_PreferredTimeframe, 2);
    double close1 = iClose(_Symbol, g_PreferredTimeframe, 2);
...
void AnalyzeMomentum()
{
    ...
    double current_range = iHigh(_Symbol, g_PreferredTimeframe, 1) - iLow(_Symbol, g_PreferredTimeframe, 1);
    double avg_atr = (atr_buffer[0] + atr_buffer[1] + atr_buffer[2]) / 3.0;

    bool is_momentum_surge = (current_range > avg_atr * 1.5);
    bool is_momentum_slow = (current_range < avg_atr * 0.5);

    // EDGE DETECTION: Momentum Surge
    if(is_momentum_surge && !g_MomentumSurgeActive)
    {
        // NEW momentum surge - find when it started
        datetime start_time = iTime(_Symbol, g_PreferredTimeframe, 1);

        // Look back to find when surge began
        for(int i = 2; i < 10; i++)
        {
            double hist_range = iHigh(_Symbol, g_PreferredTimeframe, i) - iLow(_Symbol, g_PreferredTimeframe, i);
            if(hist_range <= avg_atr * 1.5)
            {
                start_time = iTime(_Symbol, g_PreferredTimeframe, i - 1);
                break;
            }
        }

        g_MomentumSurgeStartTime = start_time;
        g_MomentumSurgeActive = true;
        g_MomentumSlowActive = false;  // Can't be both

        bool bullish = iClose(_Symbol, g_PreferredTimeframe, 1) > iOpen(_Symbol, g_PreferredTimeframe, 1);
        if(bullish)
        {
            AddPriceActionComment("⚡ MOMENTUM SURGE - Bullish acceleration detected", clrLime, PRIORITY_CRITICAL, start_time);
            AddPriceActionComment("→ Strong buying pressure - Trend may be accelerating", clrYellow, PRIORITY_IMPORTANT, start_time);
        }
        else
        {
            AddPriceActionComment("⚡ MOMENTUM SURGE - Bearish acceleration detected", clrRed, PRIORITY_CRITICAL, start_time);
            AddPriceActionComment("→ Strong selling pressure - Trend may be accelerating", clrYellow, PRIORITY_IMPORTANT, start_time);
        }
    }
    else if(!is_momentum_surge && g_MomentumSurgeActive)
    {
        // Momentum surge ended
        g_MomentumSurgeActive = false;
    }

    // EDGE DETECTION: Momentum Slowing
    if(is_momentum_slow && !g_MomentumSlowActive)
    {
        // NEW momentum slowdown - find when it started
        datetime start_time = iTime(_Symbol, g_PreferredTimeframe, 1);

        for(int i = 2; i < 10; i++)
        {
            double hist_range = iHigh(_Symbol, g_PreferredTimeframe, i) - iLow(_Symbol, g_PreferredTimeframe, i);
            if(hist_range >= avg_atr * 0.5)
            {
                start_time = iTime(_Symbol, g_PreferredTimeframe, i - 1);
                break;
            }
        ...
        g_MomentumSlowStartTime = start_time;
        g_MomentumSlowActive = true;
        g_MomentumSurgeActive = false;  // Can't be both

        AddPriceActionComment("🐌 MOMENTUM SLOWING - Reduced volatility", clrGray, PRIORITY_INFO, start_time);
        AddPriceActionComment("→ Market consolidating - Potential breakout setup forming", clrOrange, PRIORITY_INFO, start_time);
    }
    else if(!is_momentum_slow && g_MomentumSlowActive)
    {
        // Momentum slowdown ended
        g_MomentumSlowActive = false;
    }

    // Note: Volatility expanding check removed to avoid duplicate entries on every bar
    // ATR expansion is a gradual process, not a discrete event
}
...
void DetectConsolidation()
{
    ...
    double highest = iHigh(_Symbol, g_PreferredTimeframe, iHighest(_Symbol, g_PreferredTimeframe, MODE_HIGH, 10, 1));
    double lowest = iLow(_Symbol, g_PreferredTimeframe, iLowest(_Symbol, g_PreferredTimeframe, MODE_LOW, 10, 1));
    ...
    bool is_consolidating = (range < expected_range * 0.6);

    // EDGE DETECTION: Only add commentary when consolidation STARTS
    if(is_consolidating && !g_ConsolidationActive)
    {
        // NEW consolidation detected - find when it started by looking back
        datetime start_time = iTime(_Symbol, g_PreferredTimeframe, 0);

        // Look back up to 20 bars to find when consolidation began
        for(int i = 1; i < 20; i++)
        {
            double hist_high = iHigh(_Symbol, g_PreferredTimeframe, iHighest(_Symbol, g_PreferredTimeframe, MODE_HIGH, 10, i));
            double hist_low = iLow(_Symbol, g_PreferredTimeframe, iLowest(_Symbol, g_PreferredTimeframe, MODE_LOW, 10, i));
            double hist_range = hist_high - hist_low;

            // If this bar was NOT consolidating, then consolidation started after this bar
            if(hist_range >= expected_range * 0.6)
            {
                start_time = iTime(_Symbol, g_PreferredTimeframe, i - 1);
                break;
            }
        ...
        g_ConsolidationStartTime = start_time;
        g_ConsolidationActive = true;

        // Add commentary with ACCURATE timestamp of when consolidation started
        AddPriceActionComment("📦 CONSOLIDATION ZONE detected - Tight range", clrCyan, PRIORITY_CRITICAL, start_time);
        AddPriceActionComment("→ Compression phase - Breakout likely imminent", clrYellow, PRIORITY_CRITICAL, start_time);
        AddPriceActionComment("→ Range: " + DoubleToString(lowest, _Digits) + " - " + DoubleToString(highest, _Digits), clrAqua, PRIORITY_INFO, start_time);
    }
    else if(!is_consolidating && g_ConsolidationActive)
    {
        // Consolidation ended
        g_ConsolidationActive = false;
    }
}
...
void DetectBreakouts()
{
    ...
    double swing_high = iHigh(_Symbol, g_PreferredTimeframe, iHighest(_Symbol, g_PreferredTimeframe, MODE_HIGH, 20, 2));
    double swing_low = iLow(_Symbol, g_PreferredTimeframe, iLowest(_Symbol, g_PreferredTimeframe, MODE_LOW, 20, 2));
...
{
    double current_price = iClose(_Symbol, g_PreferredTimeframe, 0);
    double current_high = iHigh(_Symbol, g_PreferredTimeframe, 0);
    double current_low = iLow(_Symbol, g_PreferredTimeframe, 0);
    double prev_close = iClose(_Symbol, g_PreferredTimeframe, 1);
    ...
    for(int i = 0; i < fvg_count; i++)
    {
        ...
        if(price_in_fvg)
        {
            ...
            if(entering_from_top)
            {
                ...
                {
                    if(is_heading)
                        AddPriceActionComment("→ Price HEADING to bullish FVG [" + IntegerToString(fvg_zones[i].id) + "] from above (1st approach) - Less ideal, watch for reversal",
                                             clrOrange, PRIORITY_INFO);
                    else
                        AddPriceActionComment("→ Price REVISITING bullish FVG [" + IntegerToString(fvg_zones[i].id) + "] from above (visit #" + IntegerToString(fvg_zones[i].visit_count) + ") - Weakening zone",
                                             clrOrange, PRIORITY_INFO);
                }
                else
                {
                    if(is_heading)
                        AddPriceActionComment("→ Price HEADING to bearish FVG [" + IntegerToString(fvg_zones[i].id) + "] from above (1st approach) - IDEAL direction for shorts",
                                             clrRed, PRIORITY_INFO);
                    else
                        AddPriceActionComment("→ Price REVISITING bearish FVG [" + IntegerToString(fvg_zones[i].id) + "] from above (visit #" + IntegerToString(fvg_zones[i].visit_count) + ") - Still valid",
                                             clrRed, PRIORITY_INFO);
                }
            }
            else if(entering_from_bottom)
            {
                ...
                {
                    if(is_heading)
                        AddPriceActionComment("→ Price HEADING to bullish FVG [" + IntegerToString(fvg_zones[i].id) + "] from below (1st approach) - IDEAL direction for longs",
                                             clrLime, PRIORITY_INFO);
                    else
                        AddPriceActionComment("→ Price REVISITING bullish FVG [" + IntegerToString(fvg_zones[i].id) + "] from below (visit #" + IntegerToString(fvg_zones[i].visit_count) + ") - Still valid",
                                             clrLime, PRIORITY_INFO);
                }
                else
                {
                    if(is_heading)
                        AddPriceActionComment("→ Price HEADING to bearish FVG [" + IntegerToString(fvg_zones[i].id) + "] from below (1st approach) - Less ideal, watch for reversal",
                                             clrOrange, PRIORITY_INFO);
                    else
                        AddPriceActionComment("→ Price REVISITING bearish FVG [" + IntegerToString(fvg_zones[i].id) + "] from below (visit #" + IntegerToString(fvg_zones[i].visit_count) + ") - Weakening zone",
                                             clrOrange, PRIORITY_INFO);
                }
            }
            ...
            if(fill_percentage >= 90)
            {
                ...
                string zone_date = FormatTimeDifference(fvg_zones[i].time);
                AddPriceActionComment("✓ " + fvg_type + " FVG [" + IntegerToString(fvg_zones[i].id) + "] FULLY FILLED" + direction_text + " [" + zone_date + "]",
            ...
            {
                string zone_date = FormatTimeDifference(fvg_zones[i].time);
                AddPriceActionComment("⚡ " + fvg_type + " FVG [" + IntegerToString(fvg_zones[i].id) + "] " + DoubleToString(fill_percentage, 0) + "% FILLED" + direction_text + " [" + zone_date + "]",
            ...
            {
                AddPriceActionComment("→ " + fvg_type + " FVG [" + IntegerToString(fvg_zones[i].id) + "] touched" + direction_text + " (" + DoubleToString(fill_percentage, 0) + "% filled)",
...
void AnalyzeOrderBlockInteractions()
{
    ...
    for(int i = 0; i < ob_count; i++)
    {
        ...
        if(price_in_ob && !prev_in_ob)
        {
            ...
            string ob_date = FormatTimeDifference(order_blocks[i].time);
            AddPriceActionComment("🎯 Price " + visit_status + " " + ob_type + " ORDER BLOCK [" + IntegerToString(order_blocks[i].id) + "]" + entry_direction + visit_detail + " [" + ob_date + "]",
        ...
        {
            string ob_date = FormatTimeDifference(order_blocks[i].time);
            AddPriceActionComment("📍 Price INSIDE " + ob_type + " ORDER BLOCK - Decision zone [" + ob_date + "]",
                                 clrCyan, PRIORITY_INFO);
...
void AnalyzeLiquiditySweepsDetailed()
{
    ...
    double current_low = iLow(_Symbol, g_PreferredTimeframe, 1);
    double current_close = iClose(_Symbol, g_PreferredTimeframe, 1);
    double prev_high = iHigh(_Symbol, g_PreferredTimeframe, 2);
    double prev_low = iLow(_Symbol, g_PreferredTimeframe, 2);
...
void AnalyzePricePosition()
{
    ...
    // Price position commentary removed - it's current state, not a discrete event
    // Would create duplicate entries on every bar as price moves
    // Display this in GUI status instead of historical log

    // EDGE DETECTION: Volume Spike
    bool is_high_volume = (volume_data.volume_ratio > 2.0);
    if(is_high_volume && !g_HighVolumeActive)
    {
        // NEW volume spike detected
        datetime start_time = iTime(_Symbol, g_PreferredTimeframe, 0);

        g_HighVolumeStartTime = start_time;
        g_HighVolumeActive = true;
        g_LowVolumeActive = false;  // Can't be both

        AddPriceActionComment("🔊 VOLUME SPIKE (" + DoubleToString(volume_data.volume_ratio, 1) + "x average)",
                             clrOrange, PRIORITY_IMPORTANT, start_time);
        AddPriceActionComment("→ High institutional activity - Significant move likely incoming",
                             clrYellow, PRIORITY_INFO, start_time);
    }
    else if(!is_high_volume && g_HighVolumeActive)
    {
        // Volume spike ended
        g_HighVolumeActive = false;
    }

    // EDGE DETECTION: Low Volume
    bool is_low_volume = (volume_data.volume_ratio < 0.5);
    if(is_low_volume && !g_LowVolumeActive)
    {
        // NEW low volume detected
        datetime start_time = iTime(_Symbol, g_PreferredTimeframe, 0);

        g_LowVolumeStartTime = start_time;
        g_LowVolumeActive = true;
        g_HighVolumeActive = false;  // Can't be both

        AddPriceActionComment("🔇 Low volume (" + DoubleToString(volume_data.volume_ratio, 1) + "x average)",
                             clrGray, PRIORITY_INFO, start_time);
        AddPriceActionComment("→ Lack of conviction - Moves may be unreliable",
                             clrOrange, PRIORITY_INFO, start_time);
    }
    else if(!is_low_volume && g_LowVolumeActive)
    {
        // Low volume ended
        g_LowVolumeActive = false;
    }
}
...
{
    // CRITICAL FIX: Clear old commentary entries to prevent stale comments from cycling
    // Keep only entries from the last 2 hours to show FRESH analysis only
    datetime cutoff_time = TimeCurrent() - (2 * 3600);  // 2 hours ago
    int write_index = 0;

    // Compact the array - keep only recent entries
    for(int i = 0; i < pa_commentary_count; i++)
    {
        if(price_action_commentary[i].timestamp >= cutoff_time)
        {
            // Keep this entry - it's recent enough
            if(write_index != i)
            {
                price_action_commentary[write_index] = price_action_commentary[i];
            }
            write_index++;
        }
        // Else: skip this entry - it's too old, effectively deleting it
    }

    // Update the count to reflect removed old entries
    pa_commentary_count = write_index;
    ...
    // Session context and volatility status are displayed in the dashboard/GUI headers
